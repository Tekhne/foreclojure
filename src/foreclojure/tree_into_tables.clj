...
; your output map should be the "path" (that is, (get-in original [k1 k2])
; should be the same as (get result [k1 k2])) that you would have to take in
; the original map to get to a value, so for example {1 {2 3}} should result in
...
(defn tt [m]
  (apply merge (map #(hash-map % (get-in m %)) (for [k1 (keys m) k2 (keys (m k1))] [k1 k2]))))

; Other's:

#(into {} (for [[k v] % [vk vv] v] [[k vk] vv]))
